...
batchSize = 5
workernum = 10
subPath = Path("instance_grouping/fourth_try")
...
for epoch in range(epochs):
    ...
    for index, batch in enumerate(train):
        ...
        writer.add_image("train_node_vision",
                         torch.cat(
                             (raw_imgs[0].detach() + edges[0].detach().expand(3, -1, -1), vis, vis_edge_region_predict),
                             dim=2),
                         step)
        ...
        writer.add_image("train_image",
                         torch.cat((raw_imgs[0].detach() + edges[0].detach().expand(3, -1, -1),
                                    edge_predict[0].detach().expand(3, -1, -1)), dim=2),
                         step)
    ...
    with torch.no_grad():
        ...
        for index, batch in enumerate(val):
            ...
            vis = generator_img_by_node_feature(sorted_topk_index[0], node_output_feature[0], edge_region_predict[0],
                                                dist_map[0])
            vis_edge_region_predict = torch.nn.functional.interpolate(edge_region_predict[0:1, 0:1], size=(600, 800),
                                                                      mode="bilinear",
                                                                      align_corners=False)[0].expand(3, -1, -1)
            writer.add_image("val_step_node_vision",
                             torch.cat((raw_imgs[0].detach() + edges[0].detach().expand(3, -1, -1), vis, vis_edge_region_predict), dim=2), step)
        ...
        writer.add_image("val_image", torch.cat((raw_imgs[0].detach() + edges[0].detach().expand(3, -1, -1),
                                                ...
                                                dim=2), val_step)
        # 可视化
        vis = generator_img_by_node_feature(sorted_topk_index[0].detach(), node_output_feature[0].detach(), edge_region_predict[0].detach(),
                                            dist_map[0].detach())
        vis_edge_region_predict = torch.nn.functional.interpolate(edge_region_predict[0:1, 0:1].detach(), size=(600, 800),
                                                                  mode="bilinear",
                                                                  align_corners=False)[0].expand(3, -1, -1)
