...
class PerceptualEdgeDetection(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.fuse_conv = nn.Sequential(
            nn.Conv2d(len(self.vgg_output) * 1 + 3 + 4, 512, 3, padding=1),
        ...
        in_ch_num = len(perceptual_block_convs) * 3 * 4
        self.perceptual_fuse_1 = nn.Sequential(
            nn.Conv2d(in_ch_num, in_ch_num * 2, 1),
            nn.BatchNorm2d(in_ch_num * 2),
            nn.ReLU6(inplace=True),
            nn.Conv2d(in_ch_num * 2, in_ch_num * 2, 3, padding=1),
            nn.BatchNorm2d(in_ch_num * 2),
            nn.ReLU6(inplace=True),
            nn.Conv2d(in_ch_num * 2, in_ch_num, 1),
        )

        in_ch_num *= 2
        self.perceptual_fuse_2 = nn.Sequential(
            nn.Conv2d(in_ch_num, in_ch_num * 2, 1),
            nn.BatchNorm2d(in_ch_num * 2),
            nn.ReLU6(inplace=True),
            nn.Conv2d(in_ch_num * 2, in_ch_num * 2, 3, padding=1),
        ...
        in_ch_num *= 2
        self.perceptual_fuse_3 = nn.Sequential(
            nn.Conv2d(in_ch_num, in_ch_num * 2, 1),
            nn.BatchNorm2d(in_ch_num * 2),
            nn.ReLU6(inplace=True),
            nn.Conv2d(in_ch_num * 2, in_ch_num * 2, 3, padding=1),
            nn.BatchNorm2d(in_ch_num * 2),
            nn.ReLU6(inplace=True),
            nn.Conv2d(in_ch_num * 2, 4, 1),
        )

        self._initialize_weights(self.fuse_conv, *self.vgg_output_convs, self.perceptual_fuse_1, self.perceptual_fuse_2,
                                 self.perceptual_fuse_3)
    ...
    def forward(self, x) -> List:
        ...
        perceptual_output = []
        for i in range(4):
            for o in (self.horizontal_stroke_conv(inp),
                      self.vertical_stroke_conv(inp),
                      self.backslash_conv(inp),
                      self.forward_slash_conv(inp)):
                perceptual_output.append(nn.functional.interpolate(o, size=size, mode="bilinear"))
            inp = nn.functional.interpolate(inp, scale_factor=0.5)

        # perceptual_fuse_1 直接计算结果会导致画面白底黑线， 对输入取负后画面黑底白线
        p_inp = torch.cat(perceptual_output, 1) * -1
        perceptual_output_fuse = self.perceptual_fuse_1(p_inp)
        p_inp = torch.cat([p_inp, perceptual_output_fuse], 1)
        perceptual_output_fuse = self.perceptual_fuse_2(p_inp)
        p_inp = torch.cat([p_inp, perceptual_output_fuse], 1)
        perceptual_output_fuse = self.perceptual_fuse_3(p_inp)

        fuse = self.fuse_conv(torch.cat([x, *outputs, perceptual_output_fuse], 1))
